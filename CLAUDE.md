...
- GitHub Actions の初回ビルドが成功 (run #1, #2 とも green)。依存バージョンはすべて解決済み。
  デバッグ APK が Actions の Artifacts (`onhand-llm-debug-apk`) から取得可能
- 未検証: 実機動作

## 次にやること

1. 実機で Gemma 3 1B の読み込み〜チャット〜API〜ウィジェットを通しで動作確認
2. GPU バックエンド失敗時の CPU 自動フォールバック
3. チャット履歴の複数会話対応(会話リスト + 切替)
4. GGUF (llama.cpp) バックエンド追加の検討
